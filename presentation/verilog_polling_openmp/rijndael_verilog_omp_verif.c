...
//icpc -Wall -xHost -O2 -qopenmp rijndael_omp.c -o out2 (dont use)
//g++ -Wall -fopenmp rijndael_verilog_omp.c -o out
...
void aes_mix_columns_accel(uint8_t *state)
{
 /*   uint8_t y[16] = { 2, 3, 1, 1,  1, 2, 3, 1,  1, 1, 2, 3,  3, 1, 1, 2};
    uint8_t s[4];
    int i, j, r;
...
int aes_encrypt_accel(uint8_t *data, int len, uint8_t *key, uint8_t *w)
{
    //uint8_t w[4 * 4 * 15] = {0}; /* round key */
    uint8_t s[4 * 4] = {0}; /* state */
